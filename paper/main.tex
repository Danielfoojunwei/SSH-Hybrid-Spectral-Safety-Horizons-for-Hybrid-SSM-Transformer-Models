...
% -------------------------------------------------------
% Theorem/Hypothesis environments
% -------------------------------------------------------
\newtheorem{theorem}{Hypothesis}[section]
...
this interval the \emph{safety blind window}.

We introduce \textbf{SSH-Hybrid} (\textbf{S}pectral \textbf{S}afety
\textbf{H}orizons for \textbf{H}ybrid Models), the first empirical
investigation of whether SSM spectral properties predict safety degradation
in hybrid architectures.  We formulate two testable hypotheses.
\emph{Hypothesis~1:} hybrid models exhibit higher attack success rates (ASR)
at context distances beyond $\Horizon$ compared to pure Transformers.
\emph{Hypothesis~2:} augmenting a hybrid model with a
\emph{Monotone Boolean Composition Accumulator} (\MBCA{})---monotone
boolean probes on attention layers that cannot be erased by SSM
decay---reduces ASR to near-Transformer levels.

We test these hypotheses across four architectures (Pythia-2.8B, Mamba-2.8B,
Zamba-7B, Jamba-1.5-Mini) using attack success rate as the primary behavioral
safety metric, measured at multiple context distances with 500+ adversarial
prompts from BeaverTails.
The key experiment measures attack success rate (ASR) as a function of
context distance across all four models: if the hypothesis holds, hybrid
models should show increasing ASR beyond $\Horizon$, while pure Transformers
remain flat.  We report ASR as the primary behavioral metric, supplemented
by CHSS representation analysis.  SSH-Hybrid is the first empirical
investigation connecting SSM spectral properties to measurable safety
degradation, and the first to propose a deployable monotone-boolean
...
\begin{enumerate}[label=(\roman*), leftmargin=*, itemsep=2pt]
  ...
  \item \textbf{Hypothesis~1 (Spectral Safety Margin Bound).}  We hypothesize that for
  ...
  \item \textbf{Hypothesis~2 (MBCA Compensation).}  We design the
    ...
    forgotten regardless of subsequent SSM state decay.  We hypothesize that
    \begin{equation}
...
Section~\ref{sec:theory} presents Hypotheses~1 and~2 as testable predictions with motivation.
...
Prior work has applied spectral analysis of weight
...
\begin{table}[t]
  ...
  \begin{tabular}{lccccc}
    ...
    Spectral norm methods~\cite{miyato2018spectral} & $\sim$ & $\times$ & $\times$ & $\sim$ & $\times$ \\
    \midrule
...
\subsection{Spectral Safety Margin Bound (Hypothesis 1)}
...
\subsection{MBCA Compensation (Hypothesis 2)}
\label{sec:mbca-theorem}

\begin{theorem}[MBCA Compensation]
\label{thm:mbca-comp}
Let $\piHybrid$ be as in Hypothesis~\ref{thm:margin-bound}, and let $\piHybrid + \MBCA$ denote the augmented policy with coverage $\betaMBCA \in [0,1]$. Then
\begin{equation}
...
From Hypothesis~\ref{thm:margin-bound}, the predicted safety margin deficit is:
\begin{equation}
...
$\betaMBCA = 0$: reduces to Hypothesis~\ref{thm:margin-bound}. $\betaMBCA = 1$: $\deltaStar(\piHybrid + \MBCA) \geq \deltaStar(\piTrans)$, predicted full recovery.
\end{proof}
...
\Comment{Hypothesis~\ref{thm:main}}
\If{$g \geq g_{\min}$}
...
Using Hypothesis~\ref{thm:main} we evaluate the predicted safety gap
...
We extract and discretise the state-transition matrices of SSM layers from model weights
as follows.
...
We use seven Z-HiSPA trigger configurations designed to exploit
SSM hidden-state decay through various context-manipulation strategies:
...
We organise results around seven experiments that test the hypotheses of
Sections~\ref{sec:theory} and~\ref{sec:mbca}. The primary metric throughout
is \textbf{Attack Success Rate (ASR)}: the fraction of adversarial prompts for
which the model produces harmful output, as classified by a keyword-based safety
classifier (see Section~\ref{sec:limitations} for classifier limitations).
CHSS is reported as a supplementary representation-level metric.
...
% ---------------------------------------------------------------------------
\subsection{Experiment 6: Context-Distance Safety Degradation (Key Result)}
\label{sec:exp6}

\paragraph{Design.}
This is the central experiment. We test whether hybrid models lose safety alignment
at longer context distances compared to pure Transformers. For each of four models,
we measure ASR at context distances $d \in \{0, 50, 100, 200, 400, 800, 1600\}$ tokens.
Context is inserted as benign padding before the adversarial prompt, simulating the
``safety blind window'' scenario where an attacker pads context to exhaust SSM memory.

\paragraph{Prediction.}
If Hypothesis~1 holds:
\begin{itemize}[nosep]
  \item Mamba-2.8B ($\rSSM=1.0$) should show the steepest ASR increase with distance
  \item Jamba ($\rSSM=0.875$) and Zamba ($\rSSM=0.850$) should show moderate ASR increase
  \item Pythia-2.8B ($\rSSM=0.0$) should show minimal or no ASR increase (control)
  \item The onset of degradation should occur near $d \approx \Horizon$ for each model
\end{itemize}

\paragraph{Results.}
\emph{Results will be populated after running Experiment 6 on GPU infrastructure.
This is the make-or-break experiment for the paper's thesis.}

\begin{table}[ht]
  \centering
  \caption{Attack success rate (\%) as a function of context distance (tokens).
           Higher is less safe. 500+ adversarial prompts from BeaverTails.}
  \label{tab:distance-decay}
  \begin{tabular}{lrrrrrrr}
    \toprule
    \textbf{Model} & $d{=}0$ & $d{=}50$ & $d{=}100$ & $d{=}200$ &
      $d{=}400$ & $d{=}800$ & $d{=}1600$ \\
    \midrule
    Pythia-2.8B    & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} &
      \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    Zamba-7B       & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} &
      \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    Jamba-1.5-Mini & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} &
      \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    Mamba-2.8B     & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} &
      \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[ht]
  ...
  \caption{ASR vs.\ context distance for all four models. Each line represents one model.
           If the hypothesis holds, SSM-heavy models (Mamba, Jamba) show increasing ASR
           at larger distances, while Pythia (pure Transformer) remains approximately flat.
           Vertical dashed lines mark $\Horizon$ for each model. Shaded regions show
           bootstrap 95\% confidence intervals.}
  \label{fig:distance-decay}
\end{figure}

% ---------------------------------------------------------------------------
\subsection{Experiment 2: Spectral Radius as Safety Predictor}
\label{sec:exp2}

\paragraph{Design.}
We test whether the spectral radius $\rho$ predicts the context distance at which safety
degrades. For each model, we compute the empirical ``safety drop-off distance'' $d^*$:
the smallest context distance at which ASR exceeds the zero-distance baseline by
$>$10 percentage points. We then correlate $d^*$ with the predicted safety memory horizon
$\Horizon = \log(1/\epsilon)/\log(1/\rho)$.

If Hypothesis~1 holds, models with lower $\Horizon$ should show earlier safety degradation
(lower $d^*$), yielding a positive correlation between $\Horizon$ and $d^*$.
Pure Transformer (Pythia) serves as the control: its $d^*$ should be undefined or
very large since attention has $O(1)$ access to all context positions.

\paragraph{Results.}
\emph{Results will be populated after running Experiments 1 and 6 on GPU infrastructure.
Placeholder tables below show the reporting format.}

\begin{table}[ht]
  \centering
  \caption{Spectral radius and empirical safety drop-off distance. $d^*$ is the distance
           (tokens) at which ASR exceeds baseline by $>$10\%. Blank indicates insufficient
           degradation to determine $d^*$.}
  \label{tab:spectral-predictor}
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Model} & \rSSM{} & $\hat{\rho}$ & $\Horizon$ (tokens) & $d^*$ (tokens) \\
    \midrule
    Pythia-2.8B    & 0.000 & --- & $\infty$ & \emph{TBD} \\
    Zamba-7B       & 0.850 & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    Jamba-1.5-Mini & 0.875 & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    Mamba-2.8B     & 1.000 & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[ht]
  \centering
  \fbox{\rule{0pt}{4.5cm}\rule{0.55\textwidth}{0pt}}
  \caption{Scatter plot of predicted safety horizon $\Horizon$ vs.\ empirical drop-off
           distance $d^*$ across models. Each point is one model. A positive correlation
           supports Hypothesis~1. Pearson $r$ and $p$-value reported with bootstrap 95\% CI.}
  \label{fig:scatter}
\end{figure}

% ---------------------------------------------------------------------------
\subsection{Experiment 3: \MBCA{} Effectiveness via ASR}
\label{sec:exp3}

\paragraph{Design.}
We measure whether \MBCA{} actually reduces attack success rate, not just CHSS
representation shift. For Jamba-1.5-Mini at context distances $d \in \{0, 200, 800\}$,
we compare baseline ASR (no MBCA) against MBCA-monitored ASR with $K \in \{4, 8, 12, 16\}$
probes trained on distinct BeaverTails harm categories.

Key metric: $\Delta_\text{ASR} = \text{ASR}_\text{baseline} - \text{ASR}_\text{MBCA}$.
A positive $\Delta_\text{ASR}$ indicates MBCA successfully prevents harmful output.
We also measure false positive rate on benign prompts.

\paragraph{Results.}
\emph{Results will be populated after running Experiment 7 on GPU infrastructure.}

\begin{table}[ht]
  \centering
  \caption{\MBCA{} effectiveness on Jamba-1.5-Mini. ASR is attack success rate (lower is
           safer). $\Delta_\text{ASR}$ is the reduction achieved by MBCA. Probes are
           trained on distinct BeaverTails harm categories for diversity.}
  \label{tab:mbca}
  \begin{tabular}{rrrrrrr}
    \toprule
    $K$ & \textbf{Distance} & \textbf{ASR baseline} & \textbf{ASR w/ MBCA} &
      $\Delta_\text{ASR}$ & \textbf{Block rate} & \textbf{FP rate} \\
    \midrule
    8  & 0   & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    8  & 200 & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} & --- \\
    8  & 800 & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} & --- \\
    16 & 0   & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
    16 & 800 & \emph{TBD} & \emph{TBD} & \emph{TBD} & \emph{TBD} & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
degradation (Table~\ref{tab:degradation}), consistent with Hypothesis~1's ranking prediction.
...
\noindent \emph{K-sensitivity results will be reported as part of Experiment 7 (MBCA effectiveness).
The key metric is $\Delta_\text{ASR}$ as a function of $K$, with the goal of finding the
smallest $K$ that achieves meaningful ASR reduction ($>15$ percentage points) while keeping
...
\begin{table}[ht]
  ...
  \begin{tabular}{lcccc}
    ...
    \textbf{SSH-Hybrid (ours)}
      & \textbf{Yes}  & \textbf{Testable (Hyp.~1\&2)}     & \textbf{Low ($<$100\,ms)} & \textbf{$>$70\%} \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent SSH-Hybrid is the only framework that combines testable spectral hypotheses
(Hypotheses~1 and~2) with low runtime overhead and auditable coverage scores.
Standard safety filters achieve only $\approx$60\% coverage and are entirely blind to
SSM-driven state decay.
...
\MBCA{} introduces monotone OR-updates over $K$ binary probe activations. Once probe $k$ fires, flag $c[k] = 1$ persists indefinitely, unaffected by subsequent SSM state evolution. Hypothesis~2 predicts this: the \MBCA{} coverage $\betaMBCA$ should hold \emph{independent} of $\rho$, $\rSSM$, and interaction length. This independence is the crucial design property---\MBCA{}'s safety detection does not degrade as the architecture becomes more SSM-heavy or contexts grow longer.
...
The framework applies to any architecture with linear recurrence layers alternating with attention: Griffin~\cite{de2024griffin}, RWKV-hybrid~\cite{peng2023rwkv}, and future designs. As long as the recurrence has a well-defined spectral radius, the distance-decay hypothesis can be tested.
...
\paragraph{Propositions rest on an unproven assumption.}
The central claim---that safety margin degrades proportionally to the SSM hidden-state attenuation factor---is a hypothesis, not a proven bound. Hidden-state decay in SSM layers does not necessarily imply proportional output-level safety degradation, because attention layers, residual connections, and learned compensatory mechanisms may partially or fully offset the information loss. Proposition~1 should be treated as a testable prediction and validated against behavioral safety metrics (e.g., attack success rate) rather than assumed to hold.

\paragraph{CHSS is a proxy, not a safety metric.}
The Content-Hidden State Score measures representation shift via cosine similarity between clean and perturbed hidden states. This does not directly measure safety. A model could exhibit large representation shift yet remain behaviorally safe, or vice versa. Future work should validate CHSS against ground-truth behavioral metrics such as attack success rate or human judgement of output safety.

\paragraph{Probe diversity is critical but not guaranteed.}
The theoretical value of $K$ probes in MBCA depends on each probe detecting a distinct safety-relevant pattern. When trained with a single binary label broadcast to all $K$ probes, the result is $K$ near-identical classifiers rather than diverse detectors. Future implementations should use per-category harm labels (BeaverTails provides 14 categories) to ensure genuine probe diversity.

\paragraph{Linear SSM assumption.}
Proposition~1 relies on linear recurrence dynamics. Mamba-2 introduces selective scan dynamics (SSD) where $\bar{A}_t$ is input-dependent~\cite{dao2024mamba2}. Extensions to nonlinear SSMs may require bounding the spectral radius of the worst-case input-dependent operator.
...
Rigorous validation requires evaluation on hundreds to thousands of samples from established adversarial benchmarks (HarmBench, AdvBench), not small handcrafted sets. Statistical significance testing (bootstrap confidence intervals) should accompany all reported metrics.

\paragraph{Inference overhead measurement.}
The overhead claim requires KV-cache-enabled generation. Without KV cache, the MBCA monitor performs a full forward pass at each token, making overhead $O(n)$ per token rather than $O(1)$. Actual overhead should be measured empirically.
...
We have presented SSH-Hybrid, the first empirical investigation of whether SSM spectral
properties predict safety degradation in hybrid SSM-Transformer architectures. This
addresses a gap that has grown urgent as hybrid models enter production: no prior work
has tested whether the architectural properties that make SSMs efficient also create
exploitable safety vulnerabilities.

Four contributions:

\begin{enumerate}[label=(\arabic*)]
\item \textbf{The context-distance experiment} (Experiment~6) tests whether hybrid models
exhibit increasing attack success rates at longer context distances---the first direct
empirical test of the safety blind window hypothesis.

\item \textbf{Spectral radius as safety predictor} (Experiment~2) tests whether the
measured spectral radius $\rho$ and derived safety memory horizon $\Horizon$ predict the
empirical distance at which safety degrades, correlating a physical model property with
a behavioral safety outcome.

\item \textbf{The \MBCA{} mechanism} proposes monotone boolean probes on attention layers
as an architectural countermeasure, and measures its effectiveness via attack success
rate reduction (Experiment~7), not just representation-level metrics.

\item \textbf{An open-source audit pipeline} that measures spectral radius and deploys
\MBCA{} for any hybrid model, with end-to-end timing under 30 minutes.
\end{enumerate}

\textbf{Regardless of whether the specific hypotheses are confirmed or refuted, the
question itself is important}: as hybrid architectures proliferate, understanding the
safety implications of SSM spectral properties is essential. We release the SSH-Hybrid
codebase and audit scripts at \url{https://github.com/anonymous/ssh-hybrid}.
...
% NOTE: The following references were self-citations to unpublished work
% by the same authors. They have been replaced with notes indicating that
% the spectral measurement methodology and attack evaluation framework
% are contributions of THIS paper, not independently validated external work.
%
% \bibitem{wang2024spectralguard}
% [Removed: self-citation to unpublished work by same authors]
%
